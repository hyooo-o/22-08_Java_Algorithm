...
public class DijkstraTest {
	...
	public static void main(String[] args) throws IOException {
		...
		Arrays.fill(D, Integer.MAX_VALUE);	// D배열이 업데이트될 수 있도록 큰 값 넣고 시작
		// 출발정점 처리
		D[start] = 0;
...
//			if(minVertex == end) break;	// 문제가 start->end 로의 최단이면 탈출
			
			// step3. 선택된 정점을 경유지로 해서 미방문 정점들로 가는 비용을 따져보고 기준 최적해보다 유리하면 갱신
			for (int j = 0; j < V; j++) {
